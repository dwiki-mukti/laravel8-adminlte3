        ...
        <li class="nav-item dropdown">
            <a href="#" class="nav-link d-flex align-items-center" data-toggle="dropdown" style="gap: .3rem;">
                <div class="bg-image bg-profile"
                    style="width: 30px; background-image: url('{{ Auth::user()->avatar ?? null }}')"></div>
                <div>
                    {{ Auth::user()->name ?? 'name' }}
                </div>
            </a>
                ...
                <div class="dropdown-item">
                    <div class="media">
                        <div class="bg-image bg-profile mr-2"
                            style="width: 45px; background-image: url('{{ Auth::user()->avatar ?? null }}')"></div>
                        <div class="media-body">
                            ...
                            <p class="text-sm">{{ Auth::user()->email ?? 'email' }}</p>
                        </div>
                    </div>
                </div>
                <div class="dropdown-divider"></div>
                <div class="dropdown-item">
                    <form action="{{ route('logout') }}" method="POST" onsubmit="return confirm('Anda benar-benar ingin logout?')">
                        @csrf
